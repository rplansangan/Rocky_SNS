...
function escapeMsg(message) {
	return message.replace(/(<([^>]+)>)/ig,"")
}

$(document).ready(function(){
	var conn = new WebSocket('ws://{{ env("CHAT_SERVER") }}');
	...
	conn.onmessage = function(e) 
	{
	    ...
	    addMessageToChatBox(escapeMsg(data.msg));
	    console.log(data);
	};
	...
	$('#chatText').keyup(function(e){
		if (e.keyCode == 13) // enter was pressed
		{
			...
			var params = JSON.stringify({msg:userName+': '+message, params:message});
			conn.send(params);
			addMessageToChatBox("Me: " + escapeMsg(message));
			$(this).val("");
		}
	});
...
<div class="container" style="position:fixed;bottom:0;right:0;width:550px;height:300px;">
	<div class="row">
